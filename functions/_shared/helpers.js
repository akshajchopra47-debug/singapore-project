...
class MockSupabaseQuery {
  ...
  insert(data) {
    ...
    memoryDb[this.table].push(...inserted);
    return Promise.resolve({ data: Array.isArray(data) ? inserted : inserted[0], error: null });
  }
  update(data) {
    let items = memoryDb[this.table] || [];
    for (const filter of this.filters) {
      items = items.filter(item => String(item[filter.col]) === String(filter.val));
    }
    items.forEach(item => {
  ...
  delete() {
    const initialLen = (memoryDb[this.table] || []).length;
    let items = memoryDb[this.table] || [];
    for (const filter of this.filters) {
      memoryDb[this.table] = (memoryDb[this.table] || []).filter(item => String(item[filter.col]) !== String(filter.val));
    }
  ...
  async then(resolve, reject) {
    let items = memoryDb[this.table] || [];
    for (const filter of this.filters) {
      items = items.filter(item => String(item[filter.col]).toLowerCase() === String(filter.val).toLowerCase());
    }
    if (this.singleRow) {
      if (items.length === 0) {
