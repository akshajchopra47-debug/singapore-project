...
export async function onRequestPost(context) {
  ...
  try {
    ...
    try {
      ...
      // Unauthenticated visitor - allow chat
    }

    const body = await parseBody(request);
    const userId = user ? user.userId : (body.clientId || 'guest_user');

    // Rate limit free/guest users
    const allowed = await checkRateLimit(userId, plan, env);
    if (!allowed) {
      return err('Daily message limit reached (5/day on Free/Guest plan). Upgrade to Pro for unlimited access.', env, 429);
    }
